...
const ROLE_ICON: Record<RegisterRole, React.ElementType> = {
...
export function RegisterForm({ role }: { role: RegisterRole }) {
  ...
  const {
    ...
    defaultValues: { stage: "", year: "", section: "", governorate: "", confirmPassword: "" },
  });
  ...
  // The admin sends the parent a link carrying their child's code
  // (/register/parent?code=…); it is redeemed right after sign-up. A parent who
  // was given the code by hand can type it instead.
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      ...
      if (role === "teacher") router.push("/teacher");
        ...
        if (inviteCode) {
          try {
            const linkRes = await fetch("/api/parent/link", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ code: inviteCode }),
            });
            const linkJson = await linkRes.json();
            if (!linkRes.ok || !linkJson.success) toast.error(linkJson.error || t.codeInvalid);
          } catch { /* non-blocking; they can link from their dashboard */ }
        }
        router.push("/parent");
      }
    ...
    <div className="w-full max-w-md">
      <div className="glass rounded-2xl border border-[hsl(var(--border))] p-8 shadow-2xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          {/* Parent: the child code handed out by the administration */}
          {role === "parent" && (
            <div className="space-y-1.5">
              <Label htmlFor="inviteCode" className="flex items-center gap-1.5">
                <KeyRound className="h-3.5 w-3.5" /> {t.childCode}
              </Label>
              <Input
                id="inviteCode"
              ...
              />
              <p className="text-xs text-[hsl(var(--muted-foreground))]">{t.childCodeHelp}</p>
            </div>
          )}

          {/* Teacher: accounts are gated behind admin verification */}
          {role === "teacher" && (
            <p className="flex items-start gap-2 rounded-xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-900/10 p-3 text-xs text-[hsl(var(--muted-foreground))]">
          ...
          {/* Social sign-up always creates a student account, so it only
              belongs on the student form. */}
          {role === "student" && <SocialButtons from="/profile" />}
