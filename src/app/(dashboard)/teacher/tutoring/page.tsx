...
import { Video, Loader2, Check, X, UserPlus, Inbox } from "lucide-react";
    ...
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-[hsl(var(--muted-foreground))] mt-1">
          Accepte les demandes des élèves. Une fois acceptés, ils voient et rejoignent tes réunions.
        </p>
      </div>
        ...
        <>
          ...
          <div className="space-y-3">
            ...
            {pending.length === 0 ? (
              ...
              pending.map((r) => (
                <div key={r._id} className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4 flex items-center gap-3">
                  <Avatar className="h-10 w-10"><AvatarImage src={r.student?.avatar} /><AvatarFallback className="text-xs">{getInitials(r.student?.name || "?")}</AvatarFallback></Avatar>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium">{r.student?.name}</p>
                    <p className="text-xs text-[hsl(var(--muted-foreground))]">{r.student?.email}</p>
                  </div>
              ...
              <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] divide-y divide-[hsl(var(--border))]">
                {accepted.map((r) => (
                  <div key={r._id} className="flex items-center gap-3 p-3">
                    <Avatar className="h-9 w-9"><AvatarImage src={r.student?.avatar} /><AvatarFallback className="text-xs">{getInitials(r.student?.name || "?")}</AvatarFallback></Avatar>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium">{r.student?.name}</p>
                      <p className="text-xs text-[hsl(var(--muted-foreground))]">{r.student?.email}</p>
                    </div>
